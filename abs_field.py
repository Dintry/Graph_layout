...
def calculate_delta_value(coordinates, ideal_length, spring_strength, i):
    ...
    for j in xrange(num):
        if j != i:
            ...
            sign = np.sign(distance[j] - ideal_length[j])
            e_x[j] = sign * spring_strength[j] * (coordinates[i][0] - coordinates[j][0]) / distance[j]
            e_y[j] = sign * spring_strength[j] * (coordinates[i][1] - coordinates[j][1]) / distance[j]
...
def calculate_delta_values(coordinates, ideal_length, spring_strength):
    ...
    for i in xrange(num):
        for j in xrange(i + 1, num):
            ...
            sign = np.sign(distance[i][j] - ideal_length[i][j])
            e_x[i][j] = sign * spring_strength[i][j] * (coordinates[i][0] - coordinates[j][0]) / distance[i][j]
            e_y[i][j] = sign * spring_strength[i][j] * (coordinates[i][1] - coordinates[j][1]) / distance[i][j]
            e_x[j][i] = -e_x[i][j]
            e_y[j][i] = -e_y[i][j]
...
def calculate_displacement(coordinates, ideal_length, spring_strength, i, e_x, e_y, distance):
    ...
    for j in xrange(num):
        if j != i:
            sign = np.sign(distance[j] - ideal_length[j])
            e_xx += sign * spring_strength[j] * (1 - ((coordinates[i][0] - coordinates[j][0]) / distance[j]) ** 2) / distance[j]
            e_yy += sign * spring_strength[j] * (1 - ((coordinates[i][1] - coordinates[j][1]) / distance[j]) ** 2) / distance[j]
            addit = sign * spring_strength[j] * (coordinates[j][0] - coordinates[i][0]) * (coordinates[i][1] - coordinates[j][1]) / distance[j] ** 3
            e_xy += addit
...
def kamada_kawai(graph, options=(0.001, 1000)):
    eps, temp = options
...
def main():
    ...
    print 'Started'
    new_coordinates = kamada_kawai(graph, alg_options)
    print 'Ended'
    plt.figure(figsize=(8, 8))
